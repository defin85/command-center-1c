...
import { useSystemStatusTranslation } from '@/i18n'
    ...
    >
      <Space direction="vertical" style={{ width: '100%' }} size="small">
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          ...
          <Badge status={getStatusColor(service.status)} text={getStatusText(service.status)} />
        </Space>
          ...
          <div style={{ marginTop: 8 }}>
            {'error' in service.details && service.details.error != null && (
              <Text type="danger" style={{ fontSize: 12 }}>
                {t(($) => $.serviceCard.error, { value: String(service.details.error) })}
              </Text>
            )}
            {'version' in service.details && service.details.version != null && (
              <Text type="secondary" style={{ fontSize: 12 }}>
                {t(($) => $.serviceCard.version, { value: String(service.details.version) })}
              </Text>
            )}
          </div>
        )}
