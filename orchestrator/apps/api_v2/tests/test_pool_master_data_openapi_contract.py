...
def test_pool_master_data_paths_and_operation_ids_are_present() -> None:
    ...
    expected = {
        ...
        "/api/v2/pools/master-data/chart-import/jobs/": (
            "get",
            "v2_pools_master_data_chart_import_jobs_list",
        ),
        "/api/v2/pools/master-data/chart-import/jobs/{id}/": (
            "get",
            "v2_pools_master_data_chart_import_jobs_get",
        ),
...
def test_pool_master_data_chart_job_list_contract_has_pagination_and_problem_details() -> None:
    contract = _load_openapi_contract()
    paths = contract.get("paths")
    assert isinstance(paths, dict)

    path_item = paths.get("/api/v2/pools/master-data/chart-import/jobs/")
    assert isinstance(path_item, dict)

    get_op = path_item.get("get")
    assert isinstance(get_op, dict)
    assert get_op.get("operationId") == "v2_pools_master_data_chart_import_jobs_list"
    post_op = path_item.get("post")
    assert isinstance(post_op, dict)
    assert post_op.get("operationId") == "v2_pools_master_data_chart_import_jobs_create"

    parameters = get_op.get("parameters")
    assert isinstance(parameters, list)
    names = {str(item.get("name")) for item in parameters if isinstance(item, dict)}
    assert {"limit", "offset", "mode", "chart_source_id"}.issubset(names)
...
def test_pool_master_data_chart_contract_exposes_source_and_follower_modes() -> None:
    contract = _load_openapi_contract()

    create_schema = _load_openapi_source_schema("ChartJobCreateRequest")
    job_mode_enum = create_schema["properties"]["mode"]["enum"]
    assert job_mode_enum == [
        "preflight",
        "dry_run",
        "materialize",
        "verify_followers",
