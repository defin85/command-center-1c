...
class ClusterGroupPermission(models.Model):
    """
    Group permission for an entire cluster.
    Grants access to all databases within the cluster for members of the group.
    """
    from django.conf import settings as django_settings
...
class DatabaseGroupPermission(models.Model):
    """
    Group permission for a specific database.
    Takes precedence over cluster-level permission (max of both).
    """
    from django.conf import settings as django_settings
