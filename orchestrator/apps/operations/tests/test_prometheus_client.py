...
class TestServiceConnections:
    ...
    async def test_get_service_connections(self, client):
        ...
        with patch.object(client, 'query', new_callable=AsyncMock) as mock_query:
            mock_query.side_effect = [
                # Requests for frontend->api-gateway
                {'data': {'result': [{'value': ['1234567890', '500']}]}},
                {'data': {'result': [{'value': ['1234567890', '100']}]}},
                # Requests for api-gateway->orchestrator
                {'data': {'result': [{'value': ['1234567890', '400']}]}},
                {'data': {'result': [{'value': ['1234567890', '150']}]}},
                # ... more for each connection in SERVICE_TOPOLOGY
            ] + [
                {'data': {'result': []}}  # Default for remaining calls
            ] * 20
...
class TestServiceMetricsSerialization:
    ...
    def test_service_metrics_to_dict(self):
        """Test ServiceMetrics serialization to dictionary."""
        now = datetime.utcnow()
        ...
        assert data['ops_per_minute'] == 123.46  # Rounded to 2 decimals
        assert data['active_operations'] == 42
        assert data['p95_latency_ms'] == 500.79  # Rounded to 2 decimals
        assert data['error_rate'] == 0.0012  # Rounded to 4 decimals
...
class TestGetPrometheusClientSingleton:
    ...
    def test_get_prometheus_client_returns_singleton(self):
        """Test get_prometheus_client returns same instance."""
    ...
    def test_get_prometheus_client_multiple_calls(self):
        """Test multiple calls to get_prometheus_client."""
        import apps.operations.services.prometheus_client as prom_module
        prom_module._prometheus_client = None
