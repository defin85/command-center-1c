...
from .serializers import (
    ...
    ArtifactPurgeJobClaimSerializer,
    ArtifactPurgeJobProgressUpdateSerializer,
    ArtifactPurgeJobCompleteSerializer,
...
# =============================================================================
# Workflow Execution Endpoints (Go Workflow Engine)
# =============================================================================
...
def update_workflow_execution_status(request):
    ...
    try:
        if target_status == WorkflowExecution.STATUS_RUNNING:
            if execution.status != WorkflowExecution.STATUS_PENDING:
                return Response(
                    {'success': False, 'error': 'Execution is not pending'},
            ...
            if execution.status != WorkflowExecution.STATUS_RUNNING:
                return Response(
                    {'success': False, 'error': 'Execution is not running'},
                    status=status.HTTP_409_CONFLICT,
                )
            ...
            if execution.status not in [
                ...
                return Response(
                    {'success': False, 'error': 'Execution cannot be cancelled'},
                    status=status.HTTP_409_CONFLICT,
                )
            execution.cancel()
        else:
    ...
    logger.info(
        "Workflow execution status updated",
        extra={
            "execution_id": str(execution.id),
            "status": execution.status,
        },
...
def claim_artifact_purge_job(request):
    ...
    with transaction.atomic():
        ...
        versions = list(
            ArtifactVersion.objects.filter(artifact_id=job.artifact_id).values_list("storage_key", "size")
        )
