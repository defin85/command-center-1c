# Спецификация: api-enqueue-error-contracts

## Purpose
Фиксирует публичный контракт API v2 для ошибок при enqueue операций в Redis Streams: какие HTTP-коды возвращаются и какие `error.code` используются для клиентов (и их отражение в OpenAPI).
