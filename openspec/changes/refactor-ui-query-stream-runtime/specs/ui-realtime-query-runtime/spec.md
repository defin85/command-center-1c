...
`MainLayout`, `AuthzProvider` и другие shell-level consumers НЕ ДОЛЖНЫ (SHALL NOT) на default path определять capability доступность через набор независимых probe API calls, если тот же контекст доступен через bootstrap resource.

#### Scenario: Staff route загружает shell context через один bootstrap contract
- **GIVEN** авторизованный staff-пользователь открывает `/decisions`
- **WHEN** frontend инициализирует shell/runtime context
- **THEN** `me`, tenant/access summary и shell capability flags приходят через один canonical bootstrap path
- **AND** shell не делает отдельные capability probe calls только для построения меню и глобальных guard'ов

#### Scenario: Bootstrap failure даёт стабильный shell error state
- **GIVEN** bootstrap resource вернул transport или authorization ошибку
...
- **AND** система НЕ ДОЛЖНА запускать cascade из secondary probe errors как substitute path
