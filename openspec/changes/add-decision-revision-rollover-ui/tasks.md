...
## 2. Publish semantics
- [ ] 2.1 Переиспользовать существующий create/revise path `/api/v2/decisions` с `parent_version_id` и `database_id`, не вводя отдельный mutate endpoint без доказанной необходимости.
- [ ] 2.2 Явно зафиксировать в UI, что rollover flow создаёт новую revision и не перепривязывает существующие workflow/binding consumers автоматически.
