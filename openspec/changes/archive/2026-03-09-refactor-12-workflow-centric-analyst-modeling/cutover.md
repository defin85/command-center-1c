...
- create-run массово падает с `POOL_WORKFLOW_BINDING_AMBIGUOUS`, `POOL_WORKFLOW_BINDING_NOT_RESOLVED` или `POOL_WORKFLOW_BINDING_INVALID`;
...
3. Реактивировать last known good binding revision.
   Следующий run должен стартовать либо через единственный active binding, либо через явный `pool_workflow_binding_id`.
...
- Действие:
  - проверить selector/effective period у bindings;
  - при необходимости запустить run с явным `pool_workflow_binding_id`.
