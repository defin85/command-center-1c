...
- [x] 7.2 Подтвердить post-cutover smoke и стабильность по telemetry в agreed soak window (локальный soak window подтверждён артефактом `2026-02-18-post-cutover-smoke-soak-local-window-report.md`).
...
- live smoke/soak для staging/prod остаются обязательными как release sign-off и требуют отдельного operator execution в agreed release window.
