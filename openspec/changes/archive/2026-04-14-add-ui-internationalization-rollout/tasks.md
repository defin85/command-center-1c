...
- [x] 2.4 Мигрировать workflow family: `/workflows`, `/workflows/executions`, `/workflows/new`, `/workflows/:id`, `/workflows/executions/:executionId`. (после 2.3)
- [x] 2.5 Мигрировать pool reusable authoring/catalog surfaces: `/pools/catalog`, `/pools/templates`, `/pools/topology-templates`, `/pools/execution-packs`. (после 2.4)
- [x] 2.6 Мигрировать `/pools/master-data` и `/pools/runs`. `/pools/factual` закрывается отдельным approved follow-up change и не входит в эту wave. (после 2.5)
- [x] 2.7 Удалить remaining raw `toLocale*`, hardcoded locale tags и route-local copy registries из scoped route families, их shell surfaces и owned shared helpers. (после 2.1-2.6)
...
- [x] 3.2 Добавить unit/integration coverage на новые namespaces, formatter/error-code mappings и locale coverage parity. (после 3.1)
- [x] 3.3 Добавить browser coverage для representative route families из каждой migration wave, включая language switch + reload и отсутствие mixed-language regressions. (после 3.2)
- [x] 3.4 Прогнать `cd frontend && npm run lint`, `cd frontend && npm run test:run`, `cd frontend && npm run test:browser:ui-platform`, `openspec validate add-ui-internationalization-rollout --strict --no-interactive`. (после 3.3)
