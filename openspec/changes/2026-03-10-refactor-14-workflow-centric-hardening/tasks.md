...
- [ ] 7.4 Обновить operator-facing runbook/release notes/cutover notes под новый hardened contract, включая shared metadata snapshots, backfill, tenant-scoped cutover, decision-resource migration и rollback window.
  Фактический gap: в shipped docs остаются stale ссылки на отсутствующий путь `openspec/changes/refactor-14-workflow-centric-hardening/cutover.md` вместо активного dated change path.
