...
- [ ] 1.2 Добавить кэширование каталога (`database + auth context`) с TTL и ручным refresh-path.
- [ ] 1.3 Добавить fail-closed обработку auth/configuration ошибок для metadata-read path с machine-readable кодами.

## 2. Backend validation and contracts
- [ ] 2.1 Расширить валидацию topology mutating API: проверка ссылок `document_policy` на существующие metadata-объекты.
- [ ] 2.2 Обновить OpenAPI контракт для metadata catalog endpoint и новых validation ошибок.
- [ ] 2.3 Добавить unit/integration тесты для parser/cache/validation path.
...
- [ ] 4.3 Подготовить короткий rollout note для операторов `/pools/catalog` (как использовать builder и когда нужен raw mode).
