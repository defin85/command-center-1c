...
#### Scenario: UI использует include definition данные из list ответа
- **GIVEN** staff пользователь работает с unified list `/templates`
- **WHEN** UI загружает страницу списка
- **THEN** необходимые definition данные приходят в list response через include-механизм
