...
- minimal route semantics metadata для `platform-governed` surfaces, достаточную для inventory-driven targeting:
  - `workspace_kind` / composition family;
  - transport для route state (`search params`, path params или смешанный режим);
  - expected detail/mobile fallback contract.
...
- inventory-driven route targeting вместо ручных file globs у каждой migration wave.
...
- Слишком бедная metadata schema сведёт change обратно к hardcoded route lists в следующих waves.
...
## Migration Plan
1. Ввести inventory, tier classification и minimal route semantics metadata для `platform-governed` surfaces.
