...
Для `platform-governed` operator-facing route checked-in inventory также ДОЛЖЕН (SHALL) фиксировать минимальный набор route semantics metadata, достаточный для inventory-driven governance rules и regression targeting: тип workspace/composition, transport для route state и expected detail/mobile fallback contract.
...
#### Scenario: Platform-governed route не описал required route semantics
- **GIVEN** operator-facing route получил classification `platform-governed`
- **WHEN** checked-in governance inventory не содержит обязательную route semantics metadata
- **THEN** validation сообщает явную ошибку о неполном governance inventory
- **AND** route не считается валидно enrolled в `platform-governed` perimeter
