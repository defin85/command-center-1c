...
- [ ] 2.6 Профилировать residual worst-file tails и зафиксировать bounded split boundary для `PoolRunsPage.test.tsx` и `PoolMasterDataPage.test.tsx`.
- [ ] 2.7 Физически split `frontend/src/pages/Pools/__tests__/PoolRunsPage.test.tsx` на scenario-focused files с общим helper surface и без duplicate inventory.
- [ ] 2.8 Физически split `frontend/src/pages/Pools/__tests__/PoolMasterDataPage.test.tsx` на scenario-focused files с общим helper surface и без duplicate inventory.
...
- [ ] 4.2 Прогнать focused verification set на новых split suite files и shared helper surfaces.
- [ ] 4.3 Прогнать `cd frontend && npm run test:run` и подтвердить, что inventory не потерян, а canonical full gate завершает change без ручного прерывания.
- [ ] 4.4 Подтвердить, что heavy suites не исполняются повторно одновременно через fast и heavy perimeter после обновлённого inventory.
