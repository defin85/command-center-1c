...
Operator/API lifecycle ДОЛЖЕН (SHALL) отделять profile+draft editing от published revisions:
- `list/get/upsert` ДОЛЖНЫ (SHALL) работать на уровне profile и current draft;
- `publish` ДОЛЖЕН (SHALL) создавать новую immutable revision;
- published revision НЕ ДОЛЖНА (SHALL NOT) мутироваться через generic `upsert`;
- `list revisions` и `get revision` ДОЛЖНЫ (SHALL) быть first-class read surfaces для audit, readiness и runtime pinning.
...
#### Scenario: Upsert меняет draft, но не published revision
- **GIVEN** у `GLAccountSet` уже существует published revision, используемая runtime context
- **WHEN** оператор вызывает `upsert` для этого profile и меняет состав draft
- **THEN** current draft обновляется
- **AND** ранее published revision остаётся неизменной
- **AND** pinned runtime context не начинает ссылаться на draft state

#### Scenario: Publish создаёт новую immutable revision
- **GIVEN** оператор подготовил draft `GLAccountSet`
- **WHEN** он выполняет `publish`
- **THEN** система создаёт новую immutable revision с отдельным `revision_id`
- **AND** subsequent edits идут в draft/profile state, а не мутируют уже опубликованную revision
...
Backend-owned executable registry ДОЛЖЕН (SHALL) materialize-иться в generated shared contract/schema, который читается frontend и client contracts pipeline.
Система НЕ ДОЛЖНА (SHALL NOT) полагаться на отдельный handwritten frontend registry как на parallel source-of-truth.
Runtime endpoint МОЖЕТ (MAY) существовать как diagnostic/read-model surface, но НЕ ДОЛЖЕН (SHALL NOT) быть единственным способом доставки capability policy в UI.
...
#### Scenario: Generated registry contract держит UI и backend в одном capability source-of-truth
- **GIVEN** backend registry/type handlers уже определяют capability policy для `GLAccount` и `GLAccountSet`
- **WHEN** contracts pipeline публикует generated reusable-data registry contract
- **THEN** frontend читает эту capability policy из generated contract
- **AND** UI не поддерживает отдельный handwritten registry с потенциально другим набором флагов
