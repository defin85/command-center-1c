...
#### Scenario: Historical replay использует pinned scope contract, а не latest defaults
- **GIVEN** factual runtime artifact уже содержит pinned `GLAccountSet` revision и stable scope fingerprint
- **WHEN** выполняется replay или повторный inspect historical quarter
- **THEN** runtime использует тот же persisted scope contract
- **AND** не возвращается к hardcoded account defaults или latest profile revision
...
- `effective_members`;
- `scope_fingerprint`;
- compatibility markers `config_name`, `config_version`, `chart_identity`;
- pinned provenance `metadata_catalog_snapshot_id`, `catalog_version`, `metadata_hash`.
...
Worker и orchestrator runtime ДОЛЖНЫ (SHALL) поддерживать dual-read на период cutover: внутри текущих `v1` envelopes предпочитать nested `factual_scope_contract.v2`, если он доступен, и корректно обрабатывать legacy artifact, если он был выпущен до cutover или используется при rollback.

Система НЕ ДОЛЖНА (SHALL NOT) требовать rebuild historical checkpoints, recompute latest effective members или возврат к hardcoded account defaults только для того, чтобы пережить rollout/rollback factual scope contract.

#### Scenario: Worker предпочитает nested `factual_scope_contract.v2`, но legacy checkpoint остаётся исполнимым
- **GIVEN** новый factual checkpoint уже содержит nested `factual_scope_contract.v2` и legacy `account_codes` внутри текущего `pool_factual_sync_workflow.v1` / `pool_factual_read_lane.v1`
...
#### Scenario: Top-level factual contracts остаются `v1` на bridge-периоде
- **GIVEN** orchestrator и worker обмениваются factual artifacts в период bridge rollout
- **WHEN** в artifact появляется новый nested scope contract
- **THEN** верхнеуровневые envelopes остаются `pool_factual_sync_workflow.v1` и `pool_factual_read_lane.v1`
- **AND** версионирование scope переносится внутрь `factual_scope_contract.v2`, а не размазывается неоднозначно по нескольким top-level contract version
