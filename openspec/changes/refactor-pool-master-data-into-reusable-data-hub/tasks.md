...
- [ ] 1.1 Спроектировать executable registry-driven reusable-data model поверх текущего `Pool Master Data`, чтобы capability matrix, sync/bootstrap routing и runtime gating читались из одного backend-owned registry contract, materialized в generated shared contract для frontend и `contracts/**`.
...
- [ ] 1.3 Добавить first-class persisted storage contract для `GLAccount`, `GLAccountSet`, current draft, immutable revisions/members и type-specific binding scope для `GLAccount`, явно разделив tenant-scoped canonical identity `GLAccount` и per-infobase `ib_ref_key` / `Ref_Key` binding.
...
- [ ] 1.6 Обновить OpenAPI/contracts для новых `/api/v2/pools/master-data/gl-accounts/` и `/api/v2/pools/master-data/gl-account-sets/` surfaces, явно разделив profile+draft `upsert`, `publish`, revision read-model и generated registry contract/schema.
- [ ] 1.7 Спроектировать versioned factual scope bridge как nested `factual_scope_contract.v2` внутри стабильных `pool_factual_sync_workflow.v1` / `pool_factual_read_lane.v1`, сохранив legacy compatibility fields и target-specific `resolved_bindings` snapshot для replay-safe rollout/rollback между orchestrator и worker.
...
- [ ] 2.7 Внедрить factual scope bridge: dual-write nested `factual_scope_contract.v2` с `gl_account_set_revision_id`, `effective_members`, target-specific `resolved_bindings`, `scope_fingerprint`, provenance и `scope_contract_version`, плюс legacy `account_codes`; worker/runtime должны работать в dual-read режиме внутри текущих `v1` envelopes до cutover.
...
- [ ] 2.9 Сохранять first-class factual scope artifact с pinned `gl_account_set_revision_id`, effective member snapshot, target-specific `resolved_bindings` snapshot и stable scope fingerprint, чтобы historical replay не зависел от поздних правок профиля или latest binding re-resolve.
- [ ] 2.10 Запретить historical replay через latest binding lookup: runtime должен использовать pinned `resolved_bindings`, а при их отсутствии завершаться fail-closed с machine-readable blocker.
...
- [ ] 3.7 Показать lifecycle `GLAccountSet` как `draft + publish + immutable revisions`, чтобы оператор видел draft/latest published/pinned runtime revision и не редактировал published state через generic `upsert`.
...
- [ ] 3.9 Подключить token picker, sync affordances и bootstrap entity catalogs к generated registry contract, а не к handwritten frontend enum lists.
...
- [ ] 4.2 Добавить backend/runtime tests на factual scope bridge: nested `factual_scope_contract.v2` внутри `v1` envelopes, dual-write payload, dual-read worker compatibility, replay-safe `resolved_bindings` snapshot, fail-closed отсутствие snapshot и эквивалентность derived legacy `account_codes`.
- [ ] 4.3 Добавить frontend tests на новые workspace zones, forms, token picker, generated registry contract consumption, capability-gated sync states, draft/publish GLAccountSet flow и remediation states внутри canonical shell.
