...
Operator/API lifecycle для `GLAccountSet` в этом change фиксируется как `draft + publish`, а не как mutable latest entity:
- `list/get/upsert` работают на уровне profile и current draft;
- published revision НЕ ДОЛЖНА мутироваться через generic `upsert`;
- `publish` создаёт новую immutable revision;
- runtime, readiness и checkpoints pin-ят published revision, а не current draft.
...
- factual preflight, checkpoints и downstream runtime artifacts должны сохранять first-class machine-readable scope contract c `gl_account_set_revision_id`, `effective_members`, target-specific `resolved_bindings` и stable `scope_fingerprint`;
- изменение latest revision не должно silently менять historical replay или readiness уже созданного execution context.

`resolved_bindings` должны хранить target-specific execution snapshot, а не только ссылку на текущий binding row. Минимальный pinned payload на member/target scope:
- `database_id`;
- `canonical_gl_account_id`;
- `binding_id`;
- `chart_identity`;
- `ib_ref_key`;
- optional `predefined_data_name`.

Historical replay ДОЛЖЕН использовать pinned `resolved_bindings` snapshot как source-of-truth для уже созданного factual artifact и НЕ ДОЛЖЕН повторно резолвить latest bindings. Если snapshot отсутствует или непригоден для исполнения, runtime должен завершаться fail-closed с machine-readable blocker.
...
- versioned часть вводится как nested `factual_scope_contract.v2`, который хранит `gl_account_set_revision_id`, `effective_members`, target-specific `resolved_bindings`, `scope_fingerprint`, compatibility provenance и собственный `scope_contract_version`;
...
- worker/read runtime в bridge-периоде должен работать в dual-read режиме: внутри текущих `v1` envelopes предпочитать nested `factual_scope_contract.v2` и его pinned `resolved_bindings`, но уметь безопасно продолжать чтение legacy `account_codes`;
...
- worker publication/factual path использует nested `factual_scope_contract.v2` и его pinned `resolved_bindings` внутри текущих `v1` envelopes;
...
### Decision: Executable registry публикуется из backend source-of-truth через generated contract
Единый executable registry не должен существовать как две ручные конфигурации в backend и frontend.

Для этого change фиксируется delivery model:
- backend registry/type handlers являются единственным authoring source-of-truth;
- registry materialize'ится в generated contract/schema, который попадает в `contracts/**` и читается frontend;
- runtime endpoint МОЖЕТ существовать как diagnostic/read-model surface, но НЕ ДОЛЖЕН быть primary source-of-truth для compile-time UI behavior и capability gating.

Это позволяет UI token picker, sync affordances и bootstrap catalogs читать ту же capability policy, что и backend runtime, без drift между ручными enum lists.
...
## Migration Plan
1. Добавить executable reusable-data registry и capability hooks до расширения entity enums/API surfaces, а также materialize-ить его в generated shared contract для `contracts/**` и frontend.
...
4. Добавить first-class persisted storage contract для `GLAccount`, `GLAccountSet`, `current draft`, immutable revisions/members и `chart_identity` binding scope.
...
7. Ввести factual scope bridge: сохранить верхнеуровневые `pool_factual_sync_workflow.v1` / `pool_factual_read_lane.v1`, добавить nested `factual_scope_contract.v2` с target-specific `resolved_bindings`, dual-write legacy `account_codes`, dual-read worker/runtime и replay-safe checkpoints.
...
11. После появления canonical shell из `refactor-ui-platform-workflow-template-workspaces` расширить workspace/API/bindings без удаления текущих entity surfaces, подключить UI к generated registry contract и не допускать silent metadata fallback для account scope.
