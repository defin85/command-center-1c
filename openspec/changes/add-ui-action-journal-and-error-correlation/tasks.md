...
- [ ] 1.2 Зафиксировать canonical frontend instrumentation points: route transitions, semantic operator actions, network failures, `ErrorBoundary`, `window.onerror`, `unhandledrejection`. (после 1.1)
...
- [ ] 2.1 Зафиксировать contract для `request_id` и `ui_action_id` между shared frontend API client, API Gateway и Orchestrator logs/problem details. (после 1.1; можно выполнять параллельно с 1.2)
...
- [ ] 3.2 Добавить automated validation scope для journal schema, redaction, WebSocket reuse/churn diagnostics и end-to-end correlation plumbing на frontend/backend boundary. (после 1.2, 1.3, 2.2 и 2.3; можно выполнять параллельно с 3.1)
- [ ] 3.3 Прогнать `openspec validate add-ui-action-journal-and-error-correlation --strict --no-interactive`.
