...
Важно: change не должен пытаться закрыть за один раз все migration и UX проблемы legacy `Decision Gate`. Реалистичный scope здесь — phase 1 foundation: explicit persisted contract, fail-closed runtime semantics и minimal canonical authoring path для новых DAG.
...
- Доставить phase 1 foundation без обязательного full migration wizard.
...
- Не делать в этом change full visual conversion wizard `condition -> Decision Task + Gateway`.
- Не пытаться зафиксировать весь advanced gateway UX за пределами минимально достаточного authoring path для новых DAG.
...
2. Доставить fail-closed runtime semantics и diagnostics для новых gateway constructs.
3. Оставить legacy `condition` и raw `edge.condition` исполнимыми, но перевести их в compatibility-only/read-only authoring path.
4. Доставить minimal canonical authoring path для новых `Decision Task` / gateway DAG без обязательного full migration wizard.
5. Обновить docs и diagnostics так, чтобы новые workflow создавались только через canonical branching model.
