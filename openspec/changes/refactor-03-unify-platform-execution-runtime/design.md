...
### Decision 6: Двухтрековая реализация (independent platform track + dependent pool track)
Реализация делится на два трека:
- Track A (independent): queue-only workflow execute path, root projection в `/operations`, lane parity.
...
2. Реализовать Track A: root projection record на enqueue + queue-only workflow execute path + lane parity.
3. Подтвердить готовность upstream артефактов (`distribution_artifact.v1`, `document_plan_artifact.v1`).
4. Реализовать Track B: atomic pool graph compile на artifacts.
