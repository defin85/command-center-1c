...
Для metadata path фиксируется явная классификация:
- `401/403` от OData endpoint => `ODATA_MAPPING_NOT_CONFIGURED` (auth/configuration fail-closed);
- прочие `4xx/5xx` от OData endpoint => `POOL_METADATA_FETCH_FAILED` (upstream fetch failure);
- transport-level exception => `POOL_METADATA_FETCH_FAILED`.

Такое правило устраняет неоднозначность между auth-ошибкой и общим fetch-failure в operator-facing диагностике.

### Decision 5: TLS-only gate для production rollout
Так как Basic auth передаёт `username:password` в Base64, production rollout допускается только для OData endpoint'ов с HTTPS/TLS.

Для окружений с plain HTTP change считается no-go до включения TLS.
...
- Риск: существующие тесты/операционные ожидания для `403` могут быть привязаны к `POOL_METADATA_FETCH_FAILED`.
  - Mitigation: явно обновить тесты и rollout docs под новую классификацию `401/403 -> ODATA_MAPPING_NOT_CONFIGURED`.
...
4. Явно обновить тестовые ожидания по metadata status-классификации: `401/403 -> ODATA_MAPPING_NOT_CONFIGURED`.
5. Выполнить staging прогон с кириллическим service user в metadata refresh и publication.
6. Проверить TLS-only gate: для всех целевых OData endpoint подтверждён HTTPS/TLS; при plain HTTP rollout блокируется.
7. После подтверждения перевести change в implementation phase.
