...
Bridge server-side guardrails:
- Orchestrator валидирует соответствие `tenant_id`/`pool_run_id`/`workflow_execution_id`/`node_id` до исполнения side effect;
- при mismatch возвращается non-retryable `409 Conflict` + fail-closed код `POOL_RUNTIME_CONTEXT_MISMATCH`;
- idempotency key хранится вместе с request fingerprint;
- повтор с тем же key и иным fingerprint возвращает non-retryable `409` + `IDEMPOTENCY_KEY_CONFLICT` без side effects.
...
- для `429` transport retry учитывает `Retry-After` (если присутствует);
- общий retry budget ограничен deadline/timeout шага;
- при исчерпании budget шаг завершается fail-closed с `POOL_RUNTIME_BRIDGE_RETRY_BUDGET_EXHAUSTED`.
...
- `error_details` проходит allowlist-schema валидацию и redaction секретов;
- размер persisted `error_details` ограничен (max 8 KiB после сериализации).
...
- route decision фиксируется при старте execution (route-latching), чтобы kill-switch не переключал in-flight run в середине исполнения.
...
- Повтор idempotency key с отличающимся payload может давать неоднозначные эффекты.
  - Mitigation: fingerprint + `IDEMPOTENCY_KEY_CONFLICT` (non-retryable `409`) и контрактный тест.
- Неконсистентная tenant/run/execution связка в bridge request может нарушить tenant isolation.
  - Mitigation: обязательный server-side cross-check + `POOL_RUNTIME_CONTEXT_MISMATCH`.
...
- Игнорирование `Retry-After` и отсутствие retry budget ухудшают поведение при деградации upstream.
  - Mitigation: явная retry policy (`Retry-After` + deadline-bound budget) и тесты budget exhaustion.
...
- Kill-switch без route-latching может поменять execution path в середине run и создать nondeterministic поведение.
  - Mitigation: route-latching на старте execution; kill-switch применяется к новым run.
...
- `error_details` без лимита/редакции может утечь в API/logs.
  - Mitigation: allowlist schema + redaction + size cap + контракты на sanitization.

## Migration Plan
1. Зафиксировать в internal OpenAPI canonical bridge endpoint и status update contract (`error_code`, `error_message`, optional `error_details`), включая семантику `step_attempt`/transport retries, fingerprint/idempotency-conflict и context cross-check.
...
4. Убрать stacked retry: оставить единый retry owner для bridge/status update, реализовать `Retry-After`/budget-bound policy.
...
10. Включить rollout по feature flag/canary и kill-switch в fail-closed режиме (без legacy silent-success fallback) с route-latching для in-flight run, затем сделать default и закрыть migration window.
