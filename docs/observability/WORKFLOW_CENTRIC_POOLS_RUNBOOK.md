...
5. Во вкладке `Topology Editor` проставь на каждом active edge `document_policy_key` и добейся зелёного slot coverage относительно выбранного binding context.
...
6. Если topology/binding workspace показывает remediation backlog из-за legacy `document_policy`, выполни canonical import в `/decisions`:
   ...
   После migration проверь updated decision refs, binding slots и `document_policy_key` на affected edges.
7. Перед canary или перед разбором инцидента запусти binding preview и проверь `slot_coverage_summary`.
8. В `/pools/runs` стартуй run через явный `pool_workflow_binding_id`.
9. После старта проверь `Run Lineage / Operator Report`:
   - selected binding;
