...
## Template-oriented execution-pack compatibility

- Для новых и новых revision reusable execution packs canonical producer path остаётся `/pools/execution-packs`.
- Если create/revise execution pack возвращает `EXECUTION_PACK_TOPOLOGY_ALIAS_REQUIRED`, selected decision revision всё ещё использует concrete participant refs в topology-derived `party` или `contract` fields.
- Canonical remediation для `EXECUTION_PACK_TOPOLOGY_ALIAS_REQUIRED`:
  1. открыть `/decisions`;
  2. выпустить новую decision revision с `master_data.party.edge.*` и `master_data.contract.<id>.edge.*` aliases;
  3. вернуться в `/pools/execution-packs` и заново опубликовать immutable execution-pack revision.
- Если template-based path в `/pools/catalog` показывает `EXECUTION_PACK_TEMPLATE_INCOMPATIBLE`, structural slot coverage может быть complete, но reusable execution pack semantic incompatible для topology-aware master-data contract.
- `/pools/catalog` в этом состоянии остаётся consumer-only remediation surface: fix делается в `/pools/execution-packs` и `/decisions`, а не через pool-local override.
- Этот rollout не делает automatic rewrite historical execution packs с concrete refs.
- Operator-facing reference note: `docs/release-notes/2026-03-25-topology-aware-template-execution-packs.md`
...
4. В `/pools/catalog` открой нужный pool и во вкладке `Bindings` проверь active workflow bindings, effective period, selector scope и named publication slots.
   ...
   Если `/pools/catalog` или preview показывает `EXECUTION_PACK_TEMPLATE_INCOMPATIBLE`, remediation делается через `/pools/execution-packs` и `/decisions`, а не через pool-local attachment edit.
...
9. Перед canary или перед разбором инцидента запусти binding preview и проверь `slot_coverage_summary`.
   Если preview fail-close'ится на `EXECUTION_PACK_TEMPLATE_INCOMPATIBLE`, сначала перевыпусти reusable execution pack на topology-aware aliases.
...
- Create/revise reusable execution pack fail-close'ится на `EXECUTION_PACK_TOPOLOGY_ALIAS_REQUIRED`, если topology-derived `party` / `contract` refs остались concrete-ref-bound.
- Template-based `/pools/catalog` и `/api/v2/pools/workflow-bindings/preview/` fail-close'ятся на `EXECUTION_PACK_TEMPLATE_INCOMPATIBLE`, если reusable execution pack structurally покрывает slot-ы, но semantic incompatible по topology-aware aliases.
