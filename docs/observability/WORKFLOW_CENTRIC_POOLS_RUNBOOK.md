...
5. Во вкладке `Topology Editor` для типовой схемы сначала выбирай template-based path:
...
6. Добейся зелёного slot coverage относительно выбранного binding context.
...
7. Если topology/binding workspace показывает remediation backlog из-за legacy `document_policy`, выполни canonical import в `/decisions`:
...
8. Перед canary или перед разбором инцидента запусти binding preview и проверь `slot_coverage_summary`.
9. В `/pools/runs` стартуй run через явный `pool_workflow_binding_id`.
10. После старта проверь `Run Lineage / Operator Report`:
   - selected binding;
