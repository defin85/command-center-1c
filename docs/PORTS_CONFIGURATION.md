...
| **Worker (ops)** | 9091 | 9191 | `WORKER_PORT` |
| **Worker (workflows)** | - | 9092 | `WORKER_WORKFLOWS_METRICS_PORT` |
...
4. **`frontend/.env.local`** - Frontend host (optional, overridden by CC1C_BASE_HOST)
   ...
   # - REST: /api/* (Vite/nginx proxy -> API Gateway)
   # - WS:   /ws/*  (Vite/nginx proxy -> API Gateway)
   #
   # Прод-лайк режим (прямые запросы в API Gateway):
   # VITE_API_URL=http://${VITE_BASE_HOST}:8180/api/v2
   # VITE_WS_HOST=${VITE_BASE_HOST}:8180
   ```
