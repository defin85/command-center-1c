...
// TestStringWindows1251_Cyrillic проверяет кодирование кириллицы в Windows-1251
func TestStringWindows1251_Cyrillic(t *testing.T) {
	var buf bytes.Buffer
	enc := NewEncoder()

	// Test with Cyrillic text
	testStr := "Техническое обслуживание"
	enc.StringWindows1251(testStr, &buf)

	result := buf.Bytes()

	// Verify it's not empty
	if len(result) == 0 {
		t.Error("StringWindows1251 produced empty output")
	}

	// Verify it's Windows-1251 encoded (different from UTF-8)
	var utfBuf bytes.Buffer
	enc2 := NewEncoder()
	enc2.String(testStr, &utfBuf)

	if bytes.Equal(result, utfBuf.Bytes()) {
		t.Error("StringWindows1251 should produce different encoding than UTF-8")
	}

	t.Logf("StringWindows1251('%s'): %x", testStr, result)
}

// TestStringWindows1251_Empty проверяет что пустая строка кодируется как NULL
func TestStringWindows1251_Empty(t *testing.T) {
	var buf bytes.Buffer
	enc := NewEncoder()

	enc.StringWindows1251("", &buf)

	result := buf.Bytes()

	// Ожидаем NULL byte (0x00)
	if len(result) != 1 || result[0] != 0x00 {
		t.Errorf("Empty string should encode to NULL byte (0x00), got: %x", result)
	}

	t.Logf("StringWindows1251(''): %x (correct NULL byte)", result)
}

// TestStringWindows1251_Latin проверяет что латиница работает и в Windows-1251
func TestStringWindows1251_Latin(t *testing.T) {
	var buf bytes.Buffer
	enc := NewEncoder()
	...
	if len(result) == 0 {
		t.Error("StringWindows1251 produced empty output for Latin text")
	}

	// Verify it contains the original text (Latin chars are same in Windows-1251)
	if !bytes.Contains(result, []byte(testStr)) {
		t.Errorf("Latin text should be preserved.\nGot: %x\nWant to contain: %s",
			result, testStr)
	}

	t.Logf("StringWindows1251('%s'): %x (correct)", testStr, result)
}

// TestStringWindows1251_MixedCyrillic проверяет смешанный текст (кириллица + латиница)
func TestStringWindows1251_MixedCyrillic(t *testing.T) {
	var buf bytes.Buffer
	enc := NewEncoder()

	testStr := "Обновление (Update)"
	enc.StringWindows1251(testStr, &buf)

	result := buf.Bytes()

	if len(result) == 0 {
		t.Error("StringWindows1251 produced empty output for mixed text")
	}

	// Verify it's different from UTF-8 encoding
	var utfBuf bytes.Buffer
	enc2 := NewEncoder()
	enc2.String(testStr, &utfBuf)

	if bytes.Equal(result, utfBuf.Bytes()) {
		t.Error("StringWindows1251 should produce different encoding than UTF-8 for mixed text")
	}

	t.Logf("StringWindows1251('%s'): %x", testStr, result)
}

// TestStringWindows1251_Vs_String сравнивает поведение StringWindows1251 vs String
func TestStringWindows1251_Vs_String(t *testing.T) {
	var bufWin1251, bufString bytes.Buffer
	enc := NewEncoder()

	testStr := "Данные"

	// Кодируем через StringWindows1251
	enc.StringWindows1251(testStr, &bufWin1251)

	// Кодируем через String
	enc.String(testStr, &bufString)

	win1251Result := bufWin1251.Bytes()
	stringResult := bufString.Bytes()

	// Результаты ДОЛЖНЫ отличаться!
	if bytes.Equal(win1251Result, stringResult) {
		t.Error("StringWindows1251 should produce different encoding than String for Cyrillic text")
	}

	t.Logf("StringWindows1251('Данные'): %x", win1251Result)
	t.Logf("String('Данные'):           %x", stringResult)
}

// BenchmarkStringWindows1251_Cyrillic бенчмарк для кириллицы
func BenchmarkStringWindows1251_Cyrillic(b *testing.B) {
	var buf bytes.Buffer
	enc := NewEncoder()
	testStr := "Техническое обслуживание"
...
// BenchmarkStringWindows1251_Latin бенчмарк для латиницы
func BenchmarkStringWindows1251_Latin(b *testing.B) {
	var buf bytes.Buffer
	enc := NewEncoder()
	testStr := "Maintenance"
...
// BenchmarkStringWindows1251_Empty бенчмарк для пустой строки
func BenchmarkStringWindows1251_Empty(b *testing.B) {
	var buf bytes.Buffer
	enc := NewEncoder()
