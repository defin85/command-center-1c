...
// NewEncoder creates a new Encoder with default Codec (1.0) for testing purposes
...
func (e *encoder) String(val string, w io.Writer) {
	if len(val) == 0 {
		// RESTORE ORIGINAL BEHAVIOR: NULL for empty strings
		e.Null(w)
		return
	}
...
//
// RAS protocol expects U+FFFD (0xef 0xbf 0xbd) for empty/unknown passwords, not NULL byte.
// See: UNLOCK_BUG_PROGRESS.md for protocol sniffer traces and analysis.
