...
func NewUnblockHandler(svc SessionUnblocker, pub EventPublisher, redisClient RedisClient, metrics MetricsRecorder, timeline TimelineRecorder, credsClient CredentialsFetcher, logger *zap.Logger) *UnblockHandler {
...
func (h *UnblockHandler) HandleUnblockCommand(ctx context.Context, envelope *events.Envelope) error {
	...
	err = h.service.UnblockSessions(ctx, cmd.ClusterID, cmd.InfobaseID, dbUser, dbPwd)
